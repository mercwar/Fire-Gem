...
#include "llama.h"
#include "implant.h"  // For DisplayStatusMessage
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int llm_generate(const char *prompt, char *outbuf, size_t maxlen) {
    ...
    // Extract the required vocabulary reference from the model instance
    const struct llama_vocab *vocab = llama_model_get_vocab(model);

    int n_tokens_max = (int) strlen(prompt) + 8;
    llama_token *tokens = malloc(sizeof(llama_token) * n_tokens_max);
    if (!tokens) return -1;

    // Pass 'vocab' instead of 'ctx' to match the updated API layout
    ...
    // ✅ FIXED: Pass 'vocab' to llama_n_vocab to eliminate the C4133 warning on line 92
    int n_vocab = llama_n_vocab(vocab);

    // Pass the integer vocab size cleanly to initialization signatures
    ...
    for (int i = 0; i < max_gen_tokens; i++) {
        ...
        // Use 'llama_vocab_is_eog' with 'vocab' instead of 'llama_token_is_eog'
        if (llama_vocab_is_eog(vocab, id)) break;

        char piece[64] = {0};
        
        // Pass 'vocab' instead of 'ctx' to match signature expectations
