        ...
        case WM_COMMAND:
			...
			if (LOWORD(wParam) == 5) {
				...
				if (strlen(rawInput) > 0) {
					...
					// ✅ Modularized: use formatter instead of inline snprintf
					format_chatml_prompt(rawInput, formattedPrompt, sizeof(formattedPrompt));

					if (llm_generate(formattedPrompt, outBuf, sizeof(outBuf)) == 0) {
						// 🔗 Dispatch implant hooks before showing output
						FireGem_DispatchResponse(outBuf);
